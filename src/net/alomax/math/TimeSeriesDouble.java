...
import static net.alomax.math.TimeSeries.INVALID_AMPLITUDE;
import net.alomax.timedom.*;
import net.alomax.util.PhysicalUnits;

/**
 * double data TimeSeries object
 */
/**
 * A class representing a double data time series.
 *
 * @author Anthony Lomax
 * @version %I%, %G%
 */
public class TimeSeriesDouble extends TimeSeries {

    public static final double RANGE_MIN = -Double.MAX_VALUE;
    public static final double RANGE_MAX = Double.MAX_VALUE;
    public static final double NO_DATA = RANGE_MIN;
    public Cmplx[] cdataLocal = new Cmplx[0];
    //public static TimeSeriesDouble cdataStaticOwner = null;
    public double outOfRangeMin = RANGE_MIN;
    public double outOfRangeMax = RANGE_MAX;
    public boolean sampleChanged = true;
    /**
     * double array to contain the time series samples
     */
    public double[] sample = new double[0];
     ...
     * Copy Constructor
     */
    public TimeSeriesDouble(TimeSeriesDouble timeSeries) {

        this(timeSeries.sample, timeSeries.sampleInt, timeSeries.lagTime, timeSeries.ampUnits, timeSeries.timeUnits);
        // copy samples
        double[] timeSeriesSampleCopy = new double[sample.length];
     ...
     */
    public TimeSeriesDouble(double[] sampleDouble, double sampleInt, double lagTime, String ampUnits, String timeUnits) {
    ...
    /**
     * Sets a double array as the samples in this time series.
     *
     */
    public final void setSampleDouble(double[] newSample) {
    ...
    /**
     * Appends a double array to the samples in this time series.
     *
     */
    public final int appendDouble(int discardToIndex, double[] appendArray, int startIndexBeforeDiscard) {

        // create and load new double array
    ...
    /**
     * Returns a new double array that is a subset of the samples in this time series.
     */
    public final double[] getSampleSubsetAsDouble(int startIndex, int endIndex) {

        // create and load new double array
        double newArray[] = new double[endIndex - startIndex + 1];
        ...
        if (startIndex < 0) {
            double fillArray[] = new double[-startIndex];
        ...
        if (endIndex >= sample.length) {
            double fillArray[] = new double[endIndex - sample.length + 1];
    ...
    public final RangeDouble calculateAmplitudeRange(int indexMin, int indexMax, boolean setOutOfRangeValuesToZero) {
        ...
        RangeDouble range = new RangeDouble(Double.MAX_VALUE, -Double.MAX_VALUE);
     ...
     *
     */
    public final double[] getSampleAsDouble() {
        sampleChanged = true;
        return (sample);
    }
...
//            double value = sample[nsamp];
//            if (value > outOfRangeMin && value < outOfRangeMax) {
//                value = value - (double) (b_coeff + m_coeff * xvalue);
//                sample[nsamp] = value;
...
//
//        if (sample.length > 0) {
//            //sampleMean = (double) (mean / (double) sample.length);
//            lastSampleMean = INVALID_AMPLITUDE;
//        }
//
//    }
    ...
    public void removeMean(int nStart, int nEnd) {
        ...
        for (int nsamp = 0; nsamp < sample.length; nsamp++) {
            double value = sample[nsamp];
            if (value > outOfRangeMin && value < outOfRangeMax) {
                sample[nsamp] -= (double) sampleMean;
            }
        }
    ...
    public final void unDoRemoveMean() {
        ...
        for (int nsamp = 0; nsamp < sample.length; nsamp++) {
            double value = sample[nsamp];
            if (value > outOfRangeMin && value < outOfRangeMax) {
                sample[nsamp] += (double) lastSampleMean;
            }
        }
    ...
    public final void cutInMemory(int startIndex, int endIndex) {
        ...
        double[] cutSample = getSampleSubsetAsDouble(startIndex, endIndex);
    ...
    public final void test() {
        ...
        for (int nsamp = 0; nsamp < sample.length; nsamp++) {
            ...
            sample[nsamp] = value - (double) predNextValue;
     ...
     *
     */
    public final void integrate(double startValue) {
    ...
    public final double integrateMaxOfPosNeg(boolean peakOnly) {
        ...
        double pos_sum_max = -Double.MAX_VALUE;
        //
        boolean in_neg = false;
        int start_neg = 0;
        int first_index_neg = 0;
        int last_index_neg = 0;
        double neg_sum_max = -Double.MAX_VALUE;


        double pos_ampMin = Double.MAX_VALUE;
        double pos_ampMax = -Double.MAX_VALUE;
        double neg_ampMin = Double.MAX_VALUE;
        double neg_ampMax = -Double.MAX_VALUE;
        ...
        for (; nsamp < sample.length; nsamp++) {
            ...
            if (value >= 0.0) {
                ...
                pos_sum += value * (double) sampleInt;
                ...
                neg_sum += -value * (double) sampleInt;
    ...
    public final void differentiate(double lastValue) {
        ...
        for (int nsamp = 0; nsamp < sample.length; nsamp++) {
            double value = sample[nsamp];
            if (value > outOfRangeMin && value < outOfRangeMax) {
                diff = (value - lastValue) / (double) sampleInt;
...
/*System.out.println ("Differentiate: ampMin " + ampMin
                 ...
                 + sample[nsamp] + " lastValue " + lastValue + " (double) sampleInt "
                 + (double) sampleInt);*/
                lastValue = value;
                sample[nsamp] = diff;
    ...
    public void norm() {
        ...
        double peakValue = Math.abs(sample[indexOfAmpZeroToPeakMax()]);
        if (peakValue > 100.0f * Double.MIN_VALUE) {
    ...
    public int indexOfCumulative(double cumValue) {
        ...
        double amax = sample[indexOfAmpZeroToPeakMax()];
        int ndxcum = -1;
        double atest;
    ...
    public int[] indexOfPivotLimit(int pivotIndex, double linePivotDelay, double distanceLimit, double errorWidth) {
        ...
        double currSlope = (sample[startIndex] - sample[pivotIndex]) / (double) (startIndex - pivotIndex);
        ...
        for (int nsamp = startIndex + 1; nsamp < sample.length; nsamp++) {
            ...
            if (value > outOfRangeMin && value < outOfRangeMax) {
                ...
                double currEstValue = currSlope * (double) npts;
                double currValue = sample[nsamp] - sample[pivotIndex];
                double atest = (currEstValue - currValue) / currEstValue;
                ...
                double newSlope = (sample[nsamp] - sample[pivotIndex]) / (double) npts;
                ...
                newSlope = currSlope * ((double) (npts - 1) / (double) npts) + newSlope / (double) npts;
                currSlope = newSlope;
            }
        }
    ...
    public Cmplx[] getFFT(double taperPercent, int trailingZeroPad) {
        ...
        double[] tempSample = new double[this.sample.length + trailingZeroPad];
        System.arraycopy(this.sample, 0, tempSample, 0, this.sample.length);
        for (int i = sample.length; i < tempSample.length; i++) {
            tempSample[i] = 0.0f;
        }

        TimeSeriesDouble tempTimeSeries = new TimeSeriesDouble(tempSample, this.sampleInt, this.lagTime,
    ...
    @Override
    public void integrate(float startValue) {
        throw new UnsupportedOperationException("Not supported yet."); //To change body of generated methods, choose Tools | Templates.
    }
}	// End class TimeSeriesDouble
